...
This section returns to the question that opened the paper. Litowitz, Polson, and Sokolov [1] project that the 2028 United States AI energy allocation of 326 TWh can support roughly 6.5 × 10¹⁷ tokens per year, equivalently 225,000 tokens per resident per day. GATT v1.5.2 (post-Google-I/O 2026, May 20, 2026) measures United States per-capita throughput at **561,200 tokens per resident per day** — eighteen months *before* the paper's projection date, yet **2.49× above the paper's 2028 ceiling** (up from 2.04× at v1.4 following Pichai's I/O 2026 Gemini All-Surfaces upgrade [42]).
...
**Reading this section in v1.6: a preamble on the moving observable.** The §5.5 four-factor decomposition and the Appendix C Bayesian sensitivity analysis below were developed against the v1.4 observable of G = 459.7K, L = 225K, G/L = 2.04. The v1.5.2 observable is G = 561.2K, G/L = 2.49 — a 22% larger ratio driven primarily by the disclosed-not-editorial expansion of Gemini's All-Surfaces composite via Pichai's I/O 2026 keynote [42]. The decomposition's *structural argument* — three latent factors (GATT over-estimate, ceiling re-calibration, emitted-vs-fresh-compute units) jointly compose the ratio — is unchanged. The *parameter ranges* require refitting: at minimum, Interpretation 1's $G/G^{*}$ range tightens around 1.0 because the Gemini multiplier dependency is closed, and Interpretations 2 and 3 must jointly carry a 22% larger ratio. §5.7 (new, this revision) reports the v1.5.2 observable and flags the refit as v1.6.1 future work. The §5.5 decomposition arithmetic below retains the v1.4 numbers; readers should treat the decomposition as developed against the v1.4 cross-section, with the v1.5.2 numbers carrying the structural argument forward unchanged but the parameter ranges as estimation targets for the joint co-authored work proposed in §5.5.7.
