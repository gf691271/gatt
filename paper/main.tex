...
\begin{tabular}{llll}
\toprule
Region & Gross Token GDP (\textbackslash{}\% of \textbackslash{}\$97.0B/yr) & Value-added scalar (illustrative) & VA Token GDP share \\
\midrule
...
The 311T point estimate carries methodological uncertainty that warrants explicit bracketing. Following the parameter classification template from Inference Bottleneck \cite{inference2026bottleneck} (observed / inferred / judgment-based), GATT publishes three scenarios for the headline global figure:
...
\textbf{Best estimate} & \textbf{311} & \textbf{GATT v1.4 published values; vendor-volume-weighted, all-sources, May 11, 2026 anchored} \\
...
For the first time since GATT began publishing in February 2026, the China and United States token-volume shares are essentially tied. As of May 11 (post-v1.3 expansion that added Apple Intelligence 2T to the US side): \textbf{United States 50.2\% of global volume (156 trillion tokens per day)}, \textbf{China 49.5\% (154 trillion tokens per day)}, with Europe at ~0.24\% (0.74 trillion, dominated by Mistral and Cohere) and the Rest of World residual at the remainder. At v1.0 publication state (May 9, 2026, pre-Apple) the two were exactly tied at 154T each; the v1.3 expansion places the US marginally ahead.
...
Daily Token GDP --- vendor volume × regional blended price (Section 3.5) --- totals \textbf{\$265.8 million per day} (\$97.0 billion annualized) post-v1.3 expansion. The regional split is the inverse of volume:

\begin{itemize}
  \item \textbf{United States: \$234 million per day, 88\% of global Token GDP, on 50.2\% of volume.}
  \item \textbf{China: \$15.4 million per day, 6\% of global Token GDP, on 49.5\% of volume.}
  \item Europe: \$10.8 million per day, 4\% of Token GDP, on 0.24\% of volume.
  \item Rest of World: \$5.6 million per day, 2\% of Token GDP, on 2.0\% of volume (post-v1.3 Cohere + Sarvam additions).
\end{itemize}
...
\textbf{Confidence distribution (v1.4):} 3 High · 2 Medium-High · 8 Medium · 11 Low (totals to 24). The Gemini All Surfaces composite was downgraded from High to Medium in v1.0 (Sokolov critique on 3.2× All-Surfaces multiplier as editorial). v1.1 introduced the 4-level taxonomy (adding Medium-High between High and Medium). v1.3 added 3 Low-confidence entries (Apple Intelligence, Cohere, Sarvam) --- all judgment-based until WWDC 2026 and other vendor disclosures land.

\textbf{Parameter classification (v1.4, per Inference Bottleneck arXiv:2604.17431 \cite{inference2026bottleneck}, downsunk to vendor row in v1.1):} Each estimate is classified by source quality. \emph{Observed} (4 vendors): Doubao Volcengine official + IDC validation; OpenAI 15B tokens/min API official; Pichai Gemini 16B tokens/min API; Anthropic ARR trajectory. \emph{Inferred} (10 vendors): Microsoft, Qwen, OpenRouter, Hy3, Kimi, ERNIE, Spark, MiniMax, Perplexity, DeepSeek --- corroborated indirect signals. \emph{Judgment-based} (10 vendors): Mistral, Grok, Groq, Xiaomi MiMo, Llama API, Bedrock, GLM, plus v1.3 additions (Apple, Cohere, Sarvam). The All Surfaces multiplier for Gemini (3.2×) and Anthropic 35\% tokenizer expansion factor remain judgment-based even though the underlying vendor signals are observed.
...
\textbf{Vendor sum:} 311.43T per day across 24 vendors. The reported global total of 311T per day in the main text rounds the vendor sum. The \texttt{token\_gdp.breakdown} regional totals in the dataset (US 156T + CN 154T + EU 9T + ROW 6.15T = 325T) include a further allocation of country-level non-vendor-attributable usage; the regional sum exceeds the vendor sum by ~14T (~4.4\%). This residual is documented in the dataset's \texttt{correction\_log} and represents enterprise / consumer AI usage in tracked countries that uses non-tracked vendors or self-hosted open-weight models.
